...
async function dispatchNextUnit(
        ...
        if (sliceEntry?.done) {
          try {
            ...
            if (error instanceof MergeConflictError) {
              ...
              updateProgressWidget(ctx, fixMergeUnitType, fixMergeUnitId, state, { basePath, stepMode, autoStartTime, cmdCtx });
  ...
  updateProgressWidget(ctx, unitType, unitId, state, { basePath, stepMode, autoStartTime, cmdCtx });
  ...
  idleWatchdogHandle = setInterval(async () => {
    ...
    const recovery = await recoverTimedOutUnit(ctx, pi, unitType, unitId, "idle", {
      currentUnit, basePath, verbose, unitRecoveryCount, dispatchNextUnit,
    });
